...
switch ($action) {
  ...
  case 'Save Changes':
    ...
    if ($modified_task === null || $modified_task === false) {
      $errors[] = 'Cannot save! New Task name cannot be blank';
    } elseif ($modified_task_id === NULL || $modified_task_id === false) {
  ...
  case 'Promote Task':
    $task_index = filter_input(INPUT_POST, 'taskid', FILTER_VALIDATE_INT);
    if ($task_index === null || $task_index === false) {
      $errors[] = 'Cannot get task index for promoting!';
    } elseif ($task_index == 0) {
      $errors[] = 'The task selected is already on top (index = 0)';
    } else {
      $task_temp = $task_list[$task_index - 1];
      $task_list[$task_index - 1] = $task_list[$task_index];
      $task_list[$task_index] = $task_temp;
    }
  case 'Sort Tasks':
    sort($task_list);
    break;
